...
public abstract class PyParameterInfoTest extends LightMarkedTestCase
{
	...
	public void testStarArg()
	{
		...
		feignCtrlP(marks.get("<arg2>").getTextOffset()).check("a, b, c", new String[]{
				"b, ",
				"c"
		});
		feignCtrlP(marks.get("<arg2a>").getTextOffset()).check("a, b, c", new String[]{
				"b, ",
				"c"
		});
	}
	...
	public void testNestedMultiArg()
	{
		...
		feignCtrlP(marks.get("<arg1>").getTextOffset()).check("a, (b, c), d", new String[]{"a, "});
		feignCtrlP(marks.get("<arg23>").getTextOffset()).check("a, (b, c), d", new String[]{
				"b, ",
				"c"
		});
		feignCtrlP(marks.get("<arg4>").getTextOffset()).check("a, (b, c), d", new String[]{"d"});
	...
	public void testStarredParam()
	{
		...
		feignCtrlP(marks.get("<arg1>").getTextOffset()).check("a, b, c", new String[]{"a, "});
		feignCtrlP(marks.get("<arg23>").getTextOffset()).check("a, b, c", new String[]{
				"b, ",
				"c"
		});
	}
	...
	public void testTupleAndNamedArg1()
	{
		...
		feignCtrlP(marks.get("<arg_star>").getTextOffset()).check("a, b, c", new String[]{
				"a, ",
				"b, "
		});
	}
	...
	public void testPartialWithList()
	{ // PY-3383
		Map<String, PsiElement> marks = loadTest(2);
	...
	private Collector feignCtrlP(int offset)
	{
		...
		if(collector.getParameterOwner() != null)
		{
			...
			handler.updateUI((com.jetbrains.python.psi.PyCallExpression.PyArgumentsMapping) collector.getItemsToShow()[0], collector); // sets hint text and flags
		}
		return collector;
	}
