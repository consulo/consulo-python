...
import org.consulo.python.psi.PyStringLiteralExpression;
...
public class PyStringLiteralExpressionImpl extends PyElementImpl
	...
	public List<ASTNode> getStringNodes() {
		return Arrays.asList(getNode().getChildren(TokenSet.create(new IElementType[]{PyTokenTypes.STRING_LITERAL})));
	}
		...
		while (escMatcher.find(index)) {
			...
			if (!raw) {
				if (octal != null) {
					...
					String replacement = this.escapeMap.get(toReplace);
					if (replacement != null) {
						str = replacement;
					}
				}
