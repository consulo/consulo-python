			...
			if(!node.isNewStyleClass(myTypeEvalContext))
			{
				...
				for(PyFunction attr : node.getMethods())
				{
					if(PyNames.GETATTRIBUTE.equals(attr.getName()))
					{
						...
						registerProblem(nameNode.getPsi(), PyBundle.message("INSP.oldstyle.class.getattribute"), ProblemHighlightType.GENERIC_ERROR_OR_WARNING, null, quickFixes.toArray(new
		...
		public void visitPyCallExpression(final PyCallExpression node)
		{
			...
			if(klass != null && !klass.isNewStyleClass(myTypeEvalContext))
			{
				...
				if(PyUtil.isSuperCall(node))
				{
					...
					if(callee != null)
					{
						registerProblem(callee, PyBundle.message("INSP.oldstyle.class.super"), ProblemHighlightType.GENERIC_ERROR_OR_WARNING, null, quickFixes.toArray(quickFixes.toArray(new
								LocalQuickFix[quickFixes.size()])));
					}
				}
			}
