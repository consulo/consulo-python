...
public class PyDictKeyNamesCompletionContributor extends CompletionContributor
{
			...
			if(expr.getOperand().getText().equals(operand.getText()))
			{
				...
				if(parent instanceof PyAssignmentStatement)
				{
					if(expr.equals(((PyAssignmentStatement) parent).getLeftHandSideExpression()))
					{
						...
						if(key != null)
						{
							boolean addHandler = PsiTreeUtil.findElementOfClassAtRange(file, key.getTextRange().getStartOffset(), key.getTextRange().getEndOffset(), PyStringLiteralExpression.class)
									!= null;
	...
	public static void addDictLiteralKeys(final PyDictLiteralExpression dict, final CompletionResultSet result)
	{
		...
		{
			boolean addHandler = PsiTreeUtil.findElementOfClassAtRange(dict.getContainingFile(), expression.getTextRange().getStartOffset(), expression.getTextRange().getEndOffset(),
				...
				public void handleInsert(final InsertionContext context, final LookupElement item)
				{
					...
					if(str != null)
					{
						...
						if(isDictKeys)
						{
							...
							final boolean badQuoting = (!StringUtil.startsWithChar(str.getText(), '\'') || !StringUtil.endsWithChar(str.getText(), '\'')) && (!StringUtil.startsWithChar(str.getText()
