...
import static com.jetbrains.python.inspections.PyStringFormatParser.getEscapeRanges;
import static com.jetbrains.python.inspections.PyStringFormatParser.getFormatValueExpression;
import static com.jetbrains.python.inspections.PyStringFormatParser.getNewStyleFormatValueExpression;
...
import static com.jetbrains.python.inspections.PyStringFormatParser.substitutionsToRanges;
...
abstract public class IntroduceHandler implements RefactoringActionHandler
{
	...
	public Collection<String> getSuggestedNames(@NotNull final PyExpression expression)
	{
		...
		if(res.isEmpty())
		{  // no available names found, generate disambiguated suggestions
			for(String name : candidates)
			{
				int index = 1;
	...
	protected boolean checkIntroduceContext(PsiFile file, Editor editor, PsiElement element)
	{
		...
		{
			CommonRefactoringUtil.showErrorHint(file.getProject(), editor, PyBundle.message("refactoring.introduce.selection.error"), myDialogTitle, "refactoring.extractMethod");
			return false;
		}
		return true;
	}
