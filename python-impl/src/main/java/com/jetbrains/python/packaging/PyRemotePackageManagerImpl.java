...
public class PyRemotePackageManagerImpl extends PyPackageManagerImpl
{
		...
		{ //remote interpreter
			...
			if(manager != null)
			{
				...
				do
				{
					...
					try
					{
						processOutput = PyRemoteProcessStarterManagerUtil.getManager(remoteSdkAdditionalData).executeRemoteProcess(null, ArrayUtil.toStringArray(cmdline), workingDir, manager,
								remoteSdkAdditionalData, pathMapper, askForSudo, true);
					}
					catch(InterruptedException e)
					{
	...
	private ExecutionException analyzeException(ExecutionException exception, String command, List<String> args)
	{
		...
		{
			return new PyExecutionException("Vagrant instance is down", command, args, "", "", 0, ImmutableList.of(new LaunchVagrantFix(((VagrantNotStartedException) cause).getVagrantFolder(), (
