...
public class PythonEnterHandler extends EnterHandlerDelegateAdapter {
  ...
  public Result preprocessEnter(@Nonnull PsiFile file,
    ...
    final Boolean isSplitLine = DataManager.getInstance()
                                           .loadFromDataContext(dataContext,
    ...
    if (PyTokenTypes.TRIPLE_NODES.contains(element.getNode().getElementType()) || element.getNode()
                                                                                         .getElementType() == PyTokenTypes.DOCSTRING) {
      return Result.Continue;
    }
    ...
    if (string != null && prevElement != null && PyTokenTypes.STRING_NODES.contains(prevElement.getNode()
                                                                                               .getElementType()) && string.getTextOffset() < offset && !(element
      .getNode() instanceof
      PsiWhiteSpace)) {
  ...
  private static Result checkInsertBackslash(PsiFile file, Ref<Integer> caretOffset, DataContext dataContext, int offset, Document doc) {
    boolean autoWrapInProgress = DataManager.getInstance()
                                            .loadFromDataContext(dataContext,
                                                                 consulo.ide.impl.idea.codeInsight.editorActions.AutoHardWrapHandler.AUTO_WRAP_LINE_IN_PROGRESS_KEY) != null;
  ...
  private static void insertDocStringStub(Editor editor, PsiElement element, DocstringState state) {
    ...
    if (docOwner != null) {
      ...
      final String docString =
        PyDocstringGenerator.forDocStringOwner(docOwner).withInferredParameters(true).withQuotes(quotes).forceNewMode().buildDocString();
  ...
  private static PsiElement findWrappable(ASTNode nodeAtCaret, boolean before) {
    ...
    if (wrappable == null) {
      PsiElement emptyTuple =
        before ? findBeforeCaret(nodeAtCaret, PyTupleExpression.class) : findAfterCaret(nodeAtCaret, PyTupleExpression.class);
