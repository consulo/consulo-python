										...
										for(Map.Entry<PyExpression, PyNamedParameter> entry : mapping.getMappedParameters().entrySet())
										{
											...
											if(entry.getValue() == first_param)
											{
												...
												if(first_arg_type instanceof PyClassType)
												{
													...
													{
														registerProblem(first_arg, PyBundle.message("INSP.instance.of.$0.excpected", qual_class.getQualifiedName()));
													}
													...
													if(first_arg_class != qual_class)
													{
														...
														if(markedCallee.getCallable() instanceof PyFunction)
														{
															...
															if(callable != null)
															{
																...
																if(method != null)
																{
																	...
																	if(first_arg_class.isSubclass(qual_class, myTypeEvalContext) && calling_class.isSubclass(qual_class, myTypeEvalContext))
																	{
																		break;
														...
														// otherwise, it's not
														registerProblem(first_arg, PyBundle.message("INSP.passing.$0.instead.of.$1", first_arg_class.getQualifiedName(), qual_class.getQualifiedName
																()));
													}
												}
