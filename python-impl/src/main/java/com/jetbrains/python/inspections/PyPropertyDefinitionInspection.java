  ...
  public static class Visitor extends PyInspectionVisitor {
    ...
    private void checkForSelf(PyParameterList paramList) {
      ...
      if (parameters.length > 0 && !PyNames.CANONICAL_SELF.equals(parameters[0].getName())) {
        registerProblem(parameters[0],
                        PyBundle.message("INSP.accessor.first.param.is.$0", PyNames.CANONICAL_SELF),
                        ProblemHighlightType.WEAK_WARNING,
                        null,
                        new RenameParameterQuickFix
                          (PyNames.CANONICAL_SELF));
      }
    }

    private void checkReturnValueAllowed(@Nonnull PyCallable callable,
      ...
      if (callable instanceof PyFunction) {
        ...
        if (allowed && !someFlowHasExitPoint(function, Visitor::isAllowedExitPoint) || !allowed && someFlowHasExitPoint(function,
