...
public class PyPropertyDefinitionInspection extends PyInspection
{
	...
	public static class Visitor extends PyInspectionVisitor
	{
		...
		private void checkForSelf(PyParameterList paramList)
		{
			...
			{
				registerProblem(parameters[0], PyBundle.message("INSP.accessor.first.param.is.$0", PyNames.CANONICAL_SELF), ProblemHighlightType.WEAK_WARNING, null, new RenameParameterQuickFix
						(PyNames.CANONICAL_SELF));
			}
		}
		...
		private void checkReturnValueAllowed(@Nonnull PyCallable callable, @Nonnull PsiElement beingChecked, boolean allowed, @Nonnull String message)
		{
			if(callable instanceof PyFunction)
			{
				...
				if(allowed && !someFlowHasExitPoint(function, Visitor::isAllowedExitPoint) || !allowed && someFlowHasExitPoint(function, Visitor::isDisallowedExitPoint))
				{
		...
		private static boolean someFlowHasExitPoint(@Nonnull PyFunction function, @Nonnull Predicate<PsiElement> exitPointPredicate)
		{
			...
			ControlFlowUtil.process(ControlFlowCache.getControlFlow(function).getInstructions(), 0, instruction -> {
