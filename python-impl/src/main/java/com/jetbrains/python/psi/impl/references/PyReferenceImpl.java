...
public class PyReferenceImpl implements PsiReferenceEx, PsiPolyVariantReference
{
		...
		{ // we're a call
			...
			while(it.hasNext())
			{
				...
				if(elt instanceof PyClass)
				{
					...
					{ // init not found; maybe it's ancestor's
	...
	protected List<RatedResolveResult> resolveInner()
	{
		...
		{
			ret.addAll(Optional.ofNullable(PyBuiltinCache.getInstance(myElement).getObjectType()).map(type -> type.resolveMember(referencedName, myElement, AccessDirection.of(myElement), myContext))
