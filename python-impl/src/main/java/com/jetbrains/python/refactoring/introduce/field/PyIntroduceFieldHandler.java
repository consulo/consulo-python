...
public class PyIntroduceFieldHandler extends IntroduceHandler
{
	...
	protected boolean checkIntroduceContext(PsiFile file, Editor editor, PsiElement element)
	{
		...
		{
			CommonRefactoringUtil.showErrorHint(file.getProject(), editor, "Introduce Field refactoring cannot be used in static methods", RefactoringBundle.message("introduce.field.title"),
	...
	private static class AddFieldDeclaration implements Function<String, PyStatement>
	{
		...
		public PyStatement fun(String self_name)
		{
			...
			return PyElementGenerator.getInstance(project).createFromText(LanguageLevel.getDefault(), PyStatement.class, text.replaceFirst(PyNames.CANONICAL_SELF + "\\.", self_name + "."));
		}
	}
	...
	private static class PyInplaceFieldIntroducer extends InplaceVariableIntroducer<PsiElement>
	{
		...
		{
			super(target, operation.getEditor(), operation.getProject(), "Introduce Field", occurrences.toArray(new PsiElement[occurrences.size()]), null);
			myTarget = target;
		...
		protected void moveOffsetAfter(boolean success)
		{
			if(success && (myPanel != null && myPanel.getInitPlace() != InitPlace.SAME_METHOD) || myOperation.getInplaceInitPlace() != InitPlace.SAME_METHOD)
			{
				WriteAction.run(() -> {
					...
					{
						final PyExpression copy = PyElementGenerator.getInstance(myProject).createExpressionFromText(LanguageLevel.forElement(myTarget), myTarget.getText());
