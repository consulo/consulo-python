...
import com.jetbrains.python.impl.inspections.quickfix.PyRemoveArgumentQuickFix;
import com.jetbrains.python.impl.inspections.quickfix.PyRenameArgumentQuickFix;
import com.jetbrains.python.impl.psi.PyUtil;
import com.jetbrains.python.impl.psi.types.PyABCUtil;
...
public class PyArgumentListInspection extends PyInspection {
            ...
            for (PyDecorator deco : decorators) {
                ...
                if (markedCallee != null && !markedCallee.isImplicitlyResolved()) {
                    ...
                    if (allegedFirstParam == null || allegedFirstParam.isKeywordContainer()) {
                        ...
                        registerProblem(
                            deco,
                            PyLocalize.inspFunc$0LacksFirstArg(callable.getName()).get()
                        ); // TODO: better names for anon lambdas
                    }
                        ...
                        for (int i = firstParamOffset; i < params.size(); i += 1) {
                            ...
                            if (par == null || (!par.isKeywordContainer() && !par.isPositionalContainer() && !par.hasDefaultValue())) {
                                ...
                                registerProblem(deco, PyLocalize.inspParameter$0Unfilled(parameterName).get());
                            }
                        }
                    }
                }
                    ...
                    if (inside_type != null && !PyTypeChecker.isUnknown(inside_type)) {
                        if (((PyStarArgument) arg).isKeyword()) {
                            if (!PyABCUtil.isSubtype(inside_type, PyNames.MAPPING, context)) {
                                holder.newProblem(PyLocalize.inspExpectedDictGot$0(inside_type.getName()))
                                    .range(arg)
                                    .create();
                            }
                        }
                            ...
                            if (!PyABCUtil.isSubtype(inside_type, PyNames.ITERABLE, context)) {
                                holder.newProblem(PyLocalize.inspExpectedIterGot$0(inside_type.getName()))
                                    .range(arg)
                                    .create();
                            }
                        }
    ...
    private static void highlightParametersMismatch(
        ...
        for (PyExpression argument : mapping.getUnmappedArguments()) {
            ...
            holder.newProblem(PyLocalize.inspUnexpectedArg())
                .range(argument)
                .withFixes(quickFixes.toArray(new LocalQuickFix[quickFixes.size() - 1]))
                .create();
        }
        ...
        if (our_node != null) {
            ...
            if (close_paren != null) {
                for (PyParameter parameter : mapping.getUnmappedParameters()) {
                    ...
                    if (name != null) {
                        holder.newProblem(PyLocalize.inspParameter$0Unfilled(name))
                            .range(close_paren.getPsi())
                            .create();
                    }
                }
            }
