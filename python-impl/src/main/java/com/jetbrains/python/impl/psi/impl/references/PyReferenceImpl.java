...
public class PyReferenceImpl implements PsiReferenceEx, PsiPolyVariantReference {
  ...
  @Nonnull
  private static ResolveResultList resolveToLatestDefs(@Nonnull List<Instruction> instructions,
  ...
  private boolean resolvesToSameGlobal(PsiElement element,
                                       String elementName,
  ...
  public Object[] getVariants() {
    ...
    KeywordArgumentCompletionUtil.collectFunctionArgNames(element,
                                                          ret,
                                                          TypeEvalContext.codeCompletion(element.getProject(),
    ...
    if (underscores >= 2) {
      ...
      if (f instanceof PyFile) {
        for (String name : PyModuleType.getPossibleInstanceMembers()) {
          ret.add(LookupElementBuilder.create(name).withIcon(AllIcons.Nodes.Field));
        }
      }
    }
  ...
  public boolean equals(Object o) {
    ...
    if (!myElement.equals(that.myElement)) {
      return false;
    }
    if (!myContext.equals(that.myContext)) {
      return false;
    }
