...
public class PyMoveFileHandler extends MoveFileHandler
{
	...
	public void prepareMovedFile(PsiFile file, PsiDirectory moveDestination, Map<PsiElement, PsiElement> oldToNewMap)
	{
		if(file != null)
		{
			...
			Collection<VirtualFile> roots = PyUtil.getSourceRoots(file);
	...
	private static void updateRelativeImportsInModule(@Nonnull PyFile module)
	{
		...
		for(PyFromImportStatement statement : module.getFromImports())
		{
			...
			PsiFileSystemItem sourceElement = resolveRelativeImportFromModuleLocation(statement.getManager(), originalLocation, statement.getImportSource(), relativeLevel);
		...
		for(PyImportElement importElement : module.getImportTargets())
		{
			...
			PsiFileSystemItem resolved = resolveRelativeImportFromModuleLocation(importElement.getManager(), originalLocation, referenceExpr, 1);
			if(resolved == null)
			{
				continue;
			}
			...
			{
				ScopeOwner scopeOwner = ScopeUtil.getScopeOwner(importElement);
				if(scopeOwner == null)
				{
					continue;
				}
			...
			int relativeLevel)
	{
		...
		{
			String protocol = VirtualFileManager.extractProtocol(moduleLocation);
			assert protocol != null : "Original location: " + moduleLocation;
			String relativeImportBaseUrl = VirtualFileManager.constructUrl(protocol, relativeImportBasePath);
			VirtualFile relativeImportBaseDir = VirtualFileManager.getInstance().findFileByUrl(relativeImportBaseUrl);
	...
	public List<UsageInfo> findUsages(PsiFile file, PsiDirectory newParent, boolean searchInComments, boolean searchInNonJavaFiles)
	{
		if(file != null)
		{
			...
			List<UsageInfo> usages = PyRefactoringUtil.findUsages(file, false);
	...
	{
		Set<PsiFile> updatedFiles = new HashSet<>();
		for(UsageInfo usage : usages)
		{
			PsiElement usageElement = usage.getElement();
			if(usageElement != null)
			{
				PsiNamedElement movedElement = usageElement.getCopyableUserData(REFERENCED_ELEMENT);
				usageElement.putCopyableUserData(REFERENCED_ELEMENT, null);
				if(movedElement != null)
				{
					PsiFile usageFile = usageElement.getContainingFile();

					PyImportStatementBase importStmt = PsiTreeUtil.getParentOfType(usageElement, PyImportStatementBase.class);
					...
					if(importStmt != null)
					{
						...
						boolean usageInsideImportElement = PsiTreeUtil.getParentOfType(usageElement, PyImportElement.class) != null;
						...
						QualifiedName newElementName = QualifiedNameFinder.findCanonicalImportPath(movedElement, usageElement);
					...
					else if(usageElement instanceof PyReferenceExpression)
					{
						...
						{
							QualifiedName newQualifiedName = QualifiedNameFinder.findCanonicalImportPath(movedElement, usageElement);
							replaceWithQualifiedExpression(usageElement, newQualifiedName);
						}
						else
						{
							QualifiedName newName = QualifiedName.fromComponents(PyClassRefactoringUtil.getOriginalName(movedElement));
		...
		if(!updatedFiles.isEmpty())
		{
			...
			{
				boolean injectedFragment = InjectedLanguageManager.getInstance(file.getProject()).isInjectedFragment(file);
				if(!injectedFragment)
				{
	...
	private static PsiElement replaceWithQualifiedExpression(@Nonnull PsiElement oldElement, @Nullable QualifiedName newElementName)
	{
		...
		{
			PyElementGenerator generator = PyElementGenerator.getInstance(oldElement.getProject());
			PsiElement newElement = generator.createExpressionFromText(LanguageLevel.forElement(oldElement), newElementName.toString());
			if(newElement != null)
			{
	...
	private static Couple<PsiElement> getRelativeImportSourceRange(@Nonnull PyFromImportStatement statement)
	{
		...
		PsiElement elementAfterFrom = PsiTreeUtil.skipSiblingsForward(fromKeyword, PsiWhiteSpace.class);
