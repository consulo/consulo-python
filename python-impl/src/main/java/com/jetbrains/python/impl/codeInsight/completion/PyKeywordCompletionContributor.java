...
import com.jetbrains.python.impl.codeInsight.UnindentingInsertHandler;
...
public class PyKeywordCompletionContributor extends CompletionContributor {
            ...
            if (element instanceof PsiElement p) {
                ...
                if (container instanceof PyStatementList || container instanceof PsiFile) {
                    ...
                    while (prev instanceof PsiWhiteSpace ws) {
                        prev = ws.getPrevSibling();
                    }
                    if (prev == null) {
                        return true; // there was only whitespace before us
                    }
    ...
    */
    ...
    private static class Py3kFilter implements ElementFilter {
        ...
        @RequiredReadAction
        public boolean isAcceptable(Object element, PsiElement context) {
            return element instanceof PsiElement elem
                && elem.getContainingFile() instanceof PyFile file
                && file.getLanguageLevel().isPy3K();
        }
    ...
    private static final PsiElementPattern.Capture<PsiElement> IN_STRING_LITERAL =
        psiElement()
            .inside(PyStringLiteralExpression.class)
            .andNot(psiElement().inFile(psiFile(PyDocstringFile.class)));

    private static final ElementPattern<PsiElement> IN_FUNCTION_HEADER = or(
        psiElement()
            .inside(PyFunction.class)
            .andNot(psiElement().inside(false, psiElement(PyStatementList.class), psiElement(PyFunction.class))),
        psiElement()
            .inside(PyClass.class)
            .andNot(psiElement().inside(false, psiElement(PyStatementList.class), psiElement(PyClass.class)))
    );
    ...
    private static final PsiElementPattern.Capture<PsiElement> IN_IMPORT_AFTER_REF = psiElement().afterLeaf(
        psiElement()
    ...
    public static final PsiElementPattern.Capture<PsiElement> IN_FROM_IMPORT_AFTER_REF =
        psiElement().afterLeaf(or(
            psiElement().withElementType(PyTokenTypes.IDENTIFIER).inside(PyReferenceExpression.class),
    ...
    public static final ElementPattern<PsiElement> IN_WITH_AFTER_REF =
        psiElement().afterLeaf(
            psiElement().inside(psiElement(PyWithItem.class).with(
                new PatternCondition<>("withoutAsKeyword") {
                    @Override
                    @RequiredReadAction
    ...
    private static final PsiElementPattern.Capture<PsiElement> IN_IMPORT_STMT =
        psiElement().inside(or(
            psiElement(PyImportStatement.class),
            psiElement(PyFromImportStatement.class)
        ));
    ...
    private static final PsiElementPattern.Capture<PsiElement> AFTER_COND_STMT_NO_ELSE =
        afterStatement(
            psiElement().withChild(psiElement(PyConditionalStatementPart.class))
    ...
    private static <T extends PsiElement> PsiElementPattern.Capture<PsiElement> afterStatement(PsiElementPattern.Capture<T> statementPattern) {
        return psiElement().atStartOf(
            psiElement(PyExpressionStatement.class)
        ...
        extend(
            ...
            new CompletionProvider<CompletionParameters>() {
                protected void addCompletions(
                    ...
                    String[] spaceStrings = {"if"};
                    putKeywords(spaceStrings, TailType.SPACE, result);
                }
            }
        );
    }
    */
        ...
        extend(
            ...
            psiElement()
                ...
                .afterLeafSkipping(
                    ...
                    psiElement()
                        .inside(psiElement(PyConditionalExpression.class))
    ...
    private static class PyKeywordCompletionProvider implements CompletionProvider {
        ...
        private PyKeywordCompletionProvider(
            TailType tailType,
            @Nullable InsertHandler<PythonLookupElement> insertHandler,
            String... keywords
        ) {
            myKeywords = keywords;
            myTailType = tailType;
