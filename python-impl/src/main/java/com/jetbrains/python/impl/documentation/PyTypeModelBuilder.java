...
import static com.jetbrains.python.impl.documentation.DocumentationBuilderKit.combUp;
...
public class PyTypeModelBuilder
{
	...
	public TypeModel build(@Nullable PyType type, boolean allowUnions)
	{
		...
		else if(type instanceof PyUnionType && allowUnions)
		{
			...
			{
				result = Optional.ofNullable(getOptionalType(unionType)).<PyTypeModelBuilder.TypeModel>map(optionalType -> new OptionalType(build(optionalType, true))).orElseGet(() -> new OneOf
						(Collections2.transform(unionType.getMembers(), t -> build(t, false))));
			}
		}
