...
import static com.jetbrains.python.impl.formatter.PyCodeStyleSettings.DICT_ALIGNMENT_ON_COLON;
import static com.jetbrains.python.impl.formatter.PyCodeStyleSettings.DICT_ALIGNMENT_ON_VALUE;
import static com.jetbrains.python.impl.formatter.PythonFormattingModelBuilder.STATEMENT_OR_DECLARATION;
...
{
	private static final TokenSet ourListElementTypes = TokenSet.create(PyElementTypes.LIST_LITERAL_EXPRESSION, PyElementTypes.LIST_COMP_EXPRESSION, PyElementTypes.DICT_COMP_EXPRESSION,
			PyElementTypes.SET_COMP_EXPRESSION, PyElementTypes.DICT_LITERAL_EXPRESSION, PyElementTypes.SET_LITERAL_EXPRESSION, PyElementTypes.ARGUMENT_LIST, PyElementTypes.PARAMETER_LIST,
			PyElementTypes.TUPLE_EXPRESSION, PyElementTypes.PARENTHESIZED_EXPRESSION, PyElementTypes.SLICE_EXPRESSION, PyElementTypes.SUBSCRIPTION_EXPRESSION, PyElementTypes.GENERATOR_EXPRESSION);

	private static final TokenSet ourBrackets = TokenSet.create(PyTokenTypes.LPAR, PyTokenTypes.RPAR, PyTokenTypes.LBRACE, PyTokenTypes.RBRACE, PyTokenTypes.LBRACKET, PyTokenTypes.RBRACKET);

	private static final TokenSet ourHangingIndentOwners = TokenSet.create(PyElementTypes.LIST_LITERAL_EXPRESSION, PyElementTypes.DICT_LITERAL_EXPRESSION, PyElementTypes.SET_LITERAL_EXPRESSION,
			PyElementTypes.ARGUMENT_LIST, PyElementTypes.PARAMETER_LIST, PyElementTypes.TUPLE_EXPRESSION, PyElementTypes.PARENTHESIZED_EXPRESSION, PyElementTypes.GENERATOR_EXPRESSION, PyElementTypes
			.FUNCTION_DECLARATION, PyElementTypes.CALL_EXPRESSION, PyElementTypes.FROM_IMPORT_STATEMENT);
	...
	public PyBlock(@Nullable PyBlock parent, ASTNode node, @Nullable Alignment alignment, Indent indent, @Nullable Wrap wrap, PyBlockContext context)
	{
		myParent = parent;
	...
	@Nullable
	private PyBlock getSubBlockByNode(ASTNode node)
	{
	...
	{
		Map<ASTNode, PyBlock> blocks = new LinkedHashMap<>();
	...
	private PyBlock buildSubBlock(ASTNode child)
	{
		...
		ASTNode grandParentNode = myNode.getTreeParent();
		...
		else if(parentType == PyElementTypes.BINARY_EXPRESSION && (PythonDialectsTokenSetProvider.INSTANCE.getExpressionTokens().contains(childType) || PyTokenTypes.OPERATIONS.contains(childType)))
		{
			...
			{
				PyParenthesizedExpression parens = PsiTreeUtil.getParentOfType(myNode.getPsi(), PyParenthesizedExpression.class, true, PyStatementPart.class);
		...
		{
			PyExpression indexExpression = ((PySubscriptionExpression) myNode.getPsi()).getIndexExpression();
	...
	private static boolean isValueOfKeyValuePairOfDictLiteral(ASTNode node)
	{
		return isValueOfKeyValuePair(node) && isChildOfDictLiteral(node.getTreeParent());
	}

	private static boolean isChildOfKeyValuePairOfDictLiteral(ASTNode node)
	{
		return isChildOfKeyValuePair(node) && isChildOfDictLiteral(node.getTreeParent());
	}

	private static boolean isChildOfDictLiteral(ASTNode node)
	{
	...
	private boolean argumentMayHaveSameIndentAsFollowingStatementList()
	{
		...
		PsiElement header = getControlStatementHeader(myNode);
	...
	private static boolean hasHangingIndent(PsiElement elem)
	{
		...
		if(ourHangingIndentOwners.contains(elementType) && PyTokenTypes.OPEN_BRACES.contains(firstChildNode.getElementType()))
		{
			...
			PsiElement[] items = getItems(elem);
			if(items.length == 0)
			{
	...
	private static PsiElement[] getItems(PsiElement elem)
	{
	...
	@Nullable
	private static PsiElement getControlStatementHeader(ASTNode node)
	{
		PyStatementPart statementPart = PsiTreeUtil.getParentOfType(node.getPsi(), PyStatementPart.class, false, PyStatementList.class);
		if(statementPart != null)
		{
			return statementPart;
		}
		PyWithItem withItem = PsiTreeUtil.getParentOfType(node.getPsi(), PyWithItem.class);
		if(withItem != null)
		{
	...
	private boolean isSliceOperand(ASTNode child)
	{
		...
		{
			PySliceExpression sliceExpression = (PySliceExpression) myNode.getPsi();
	...
	private static boolean hasLineBreaksBeforeInSameParent(ASTNode node, int minCount)
	{
	...
	private static boolean hasLineBreakAfterIgnoringComments(ASTNode node)
	{
	...
	public Spacing getSpacing(@Nullable Block child1, Block child2)
	{
		if(child1 instanceof ASTBlock && child2 instanceof ASTBlock)
		{
			...
			{
				PsiElement nonCommentAfter = PyPsiUtils.getNextNonCommentSibling(psi2, true);
			...
			{
				PyNamedParameter namedParameter = as(myNode.getPsi(), PyNamedParameter.class);
			...
			if((PyElementTypes.CLASS_OR_FUNCTION.contains(childType1) && STATEMENT_OR_DECLARATION.contains(childType2)) || STATEMENT_OR_DECLARATION.contains(childType1) && PyElementTypes
					.CLASS_OR_FUNCTION.contains(childType2))
			{
			...
			if(psi1 instanceof PyImportStatementBase)
			{
				...
				{
					Boolean leftImportIsGroupStart = psi1.getCopyableUserData(IMPORT_GROUP_BEGIN);
					Boolean rightImportIsGroupStart = psi2.getCopyableUserData(IMPORT_GROUP_BEGIN);
	...
	private Spacing getBlankLinesForOption(int option)
	{
		...
		return Spacing.createSpacing(0, 0, blankLines, myContext.getSettings().KEEP_LINE_BREAKS, myContext.getSettings().KEEP_BLANK_LINES_IN_DECLARATIONS);
	}

	private boolean needLineBreakInStatement()
	{
		PyStatement statement = PsiTreeUtil.getParentOfType(myNode.getPsi(), PyStatement.class);
		if(statement != null)
		{
			Collection<PyStatementPart> parts = PsiTreeUtil.collectElementsOfType(statement, PyStatementPart.class);
			return (parts.size() == 1 && myContext.getPySettings().NEW_LINE_AFTER_COLON) || (parts.size() > 1 && myContext.getPySettings().NEW_LINE_AFTER_COLON_MULTI_CLAUSE);
		}
		return false;
	}
	...
	public ChildAttributes getChildAttributes(int newChildIndex)
	{
		...
		if(newChildIndex > 0)
		{
			...
			PyBlock insertAfterBlock = getSubBlockByIndex(newChildIndex - 1);
		...
		{ // was 1... strange
			@SuppressWarnings("ConstantConditions") int indent = myContext.getSettings().getIndentOptions().INDENT_SIZE;
    ...
    */
	...
	private Alignment getChildAlignment()
	{
		if(ourListElementTypes.contains(myNode.getElementType()))
		{
			...
			{
				PyKeyValueExpression lastElement = ArrayUtil.getLastElement(((PyDictLiteralExpression) myNode.getPsi()).getElements());
	...
	private Indent getChildIndent(int newChildIndex)
	{
		...
		else if(lastChild != null && PyElementTypes.LIST_LIKE_EXPRESSIONS.contains(lastChild.getElementType()))
		{
			...
			if(lastLastChild != null && lastLastChild.getPsi() instanceof PsiErrorElement)
			{
				...
				boolean found = false;
				while(exprItem != null)
				{ // find a worthy element to align to
				...
				if(found)
				{
					...
					if(doc != null)
					{
						...
						int itemCol = exprItem.getTextOffset() - doc.getLineStartOffset(lineNum);
						PsiElement hereElt = getNode().getPsi();
						lineNum = doc.getLineNumber(hereElt.getTextOffset());
						int nodeCol = hereElt.getTextOffset() - doc.getLineStartOffset(lineNum);
						int padding = itemCol - nodeCol;
						if(padding > 0)
						{ // negative is a syntax error,  but possible
	...
	{
		if(newChildIndex == 0)
		{  // block text contains backslash line wrappings, child block list not built
			return null;
		}
	...
	public boolean isIncomplete()
	{
		...
		{
			PyArgumentList argumentList = (PyArgumentList) myNode.getPsi();
