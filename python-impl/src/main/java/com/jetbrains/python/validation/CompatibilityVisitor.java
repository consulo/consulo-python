...
public abstract class CompatibilityVisitor extends PyAnnotator
{
	...
	public void visitPyAnnotation(PyAnnotation node)
	{
		...
		{
			registerForAllMatchingVersions(level -> level.isOlderThan(LanguageLevel.PYTHON36), " not support variable annotations", node, null);
		}
	}
	...
	public void visitPyDictCompExpression(PyDictCompExpression node)
	{
		...
		registerForAllMatchingVersions(level -> !level.supportsSetLiterals(), " not support dictionary comprehensions", node, new ConvertDictCompQuickFix(), false);
	}
	...
	public void visitPySetLiteralExpression(PySetLiteralExpression node)
	{
		...
		registerForAllMatchingVersions(level -> !level.supportsSetLiterals(), " not support set literal expressions", node, new ConvertSetLiteralQuickFix(), false);
	}
	...
	public void visitPySetCompExpression(PySetCompExpression node)
	{
		...
		registerForAllMatchingVersions(level -> !level.supportsSetLiterals(), " not support set comprehensions", node, null, false);
	}
	...
	public void visitPyStarExpression(PyStarExpression node)
	{
		...
		{
			registerOnFirstMatchingVersion(level -> level.isOlderThan(LanguageLevel.PYTHON30), "Python versions < 3.0 do not support starred expressions as assignment targets", node);
		}

		if(node.isUnpacking())
		{
			registerOnFirstMatchingVersion(level -> level.isOlderThan(LanguageLevel.PYTHON35), "Python versions < 3.5 do not support starred expressions in tuples, lists, and sets", node);
		}
	}
	...
	public void visitPyDoubleStarExpression(PyDoubleStarExpression node)
	{
		...
		registerOnFirstMatchingVersion(level -> level.isOlderThan(LanguageLevel.PYTHON35), "Python versions < 3.5 do not support starred expressions in dicts", node);
	}
	...
	{
		registerOnFirstMatchingVersion(level -> level.isOlderThan(LanguageLevel.PYTHON35), "Python versions < 3.5 do not support matrix multiplication operators", node);
	}
	...
	public void visitPyNumericLiteralExpression(final PyNumericLiteralExpression node)
	{
		...
		if(node.isIntegerLiteral())
		{
			...
			{
				registerForAllMatchingVersions(LanguageLevel::isPy3K, " not support a trailing \'l\' or \'L\'.", node, new RemoveTrailingLQuickFix());
			}
			...
			if(text.length() > 1 && text.charAt(0) == '0')
			{
				...
				{
					registerForAllMatchingVersions(LanguageLevel::isPy3K, " not support this syntax. It requires '0o' prefix for octal literals", node, new ReplaceOctalNumericLiteralQuickFix());
				}
			}
		}
		...
		if(text.contains("_"))
		{
			registerForAllMatchingVersions(level -> level.isOlderThan(LanguageLevel.PYTHON36), " not support underscores in numeric literals", node, new PyRemoveUnderscoresInNumericLiteralsQuickFix
					());
		}
	}
	...
	public void visitPyStringLiteralExpression(final PyStringLiteralExpression node)
	{
		...
		for(ASTNode stringNode : node.getStringNodes())
		{
			...
			registerForAllMatchingVersions(level -> !AVAILABLE_PREFIXES.getOrDefault(level, DEFAULT_PREFIXES).contains(prefix), " not support a '" + prefix + "' prefix", node, range, new
					RemovePrefixQuickFix(prefix), true);
		}
	}
	...
	public void visitPyListCompExpression(final PyListCompExpression node)
	{
		...
		registerForAllMatchingVersions(level -> UnsupportedFeaturesUtil.visitPyListCompExpression(node, level), " not support this syntax in list comprehensions.", nodes, new
	...
	public void visitPyRaiseStatement(PyRaiseStatement node)
	{
		...
		// empty raise
		registerForAllMatchingVersions(level -> UnsupportedFeaturesUtil.raiseHasNoArgs(node, level), " not support this syntax. Raise with no arguments can only be used in an except block", node,
				null, false);

		// raise 1, 2, 3
		registerForAllMatchingVersions(level -> UnsupportedFeaturesUtil.raiseHasMoreThenOneArg(node, level), " not support this syntax.", node, new ReplaceRaiseStatementQuickFix());

		// raise exception from cause
		registerForAllMatchingVersions(level -> UnsupportedFeaturesUtil.raiseHasFromKeyword(node, level), " not support this syntax.", node, new ReplaceRaiseStatementQuickFix());
	}
	...
	public void visitPyReprExpression(PyReprExpression node)
	{
		...
		registerForAllMatchingVersions(LanguageLevel::isPy3K, " not support backquotes, use repr() instead", node, new ReplaceBackquoteExpressionQuickFix());
	}
	...
	public void visitPyWithStatement(PyWithStatement node)
	{
		...
		if(items.length > 1)
		{
			registerForAllMatchingVersions(level -> !level.supportsSetLiterals() && !level.equals(LanguageLevel.PYTHON24), " not support multiple context managers", Arrays.asList(items).subList(1,
					items.length), null);
		}
	...
	public void visitPyPrintStatement(PyPrintStatement node)
	{
		...
		final Predicate<PsiElement> nonParenthesesPredicate = element -> !(element instanceof PyParenthesizedExpression || element instanceof PyTupleExpression);

		if(arguments.length == 0 || Arrays.stream(arguments).anyMatch(nonParenthesesPredicate))
		{
			registerOnFirstMatchingVersion(LanguageLevel::isPy3K, "Python version >= 3.0 do not support this syntax. " + "The print statement has been replaced with a print() function", node, new
					CompatibilityPrintCallQuickFix());
		}
	}
	...
	public void visitPyAssignmentStatement(PyAssignmentStatement node)
	{
		...
		if(myVersionsToProcess.contains(LanguageLevel.PYTHON24))
		{
			...
			while(!st.isEmpty())
			{
				...
				{
					registerProblem(node, "Python version 2.4 doesn't support this syntax. " + "In Python <= 2.4, yield was a statement; it didn't return any value.");
				}
				else
				{
	...
	{ // PY-2795
		...
		{
			registerOnFirstMatchingVersion(LanguageLevel.PYTHON24::equals, "Python version 2.4 doesn't support this syntax. You could use a finally block to ensure " + "that code is always executed," +
					" or one or more except blocks to catch specific exceptions.", node);
		}
	}
	...
	public void visitPyCallExpression(PyCallExpression node)
	{
		...
		{
			registerForAllMatchingVersions(level -> !level.isPy3K(), " not support this syntax. super() should have arguments in Python 2", node, null);
		}
	...
	public void visitPyPrefixExpression(PyPrefixExpression node)
	{
		...
		{
			registerOnFirstMatchingVersion(level -> level.isOlderThan(LanguageLevel.PYTHON35), "Python versions < 3.5 do not support this syntax", node);
		}
	}
	...
	public void visitPyYieldExpression(PyYieldExpression node)
	{
		...
		Optional.ofNullable(ScopeUtil.getScopeOwner(node)).map(owner -> PyUtil.as(owner, PyFunction.class)).filter(function -> function.isAsync() && function.isAsyncAllowed()).ifPresent(function -> {
		...
		registerOnFirstMatchingVersion(level -> level.isOlderThan(LanguageLevel.PYTHON33), "Python versions < 3.3 do not support this syntax. Delegating to a subgenerator is available since " +
				"Python 3.3; use explicit iteration over subgenerator instead.", node);
	}
	...
	public void visitPyNoneLiteralExpression(PyNoneLiteralExpression node)
	{
		if(node.isEllipsis())
		{
			...
			registerOnFirstMatchingVersion(level -> level.isOlderThan(LanguageLevel.PYTHON30), "Python versions < 3.0 do not support '...' outside of sequence slicings.", node);
		}
	}
	...
	private void checkAsyncKeyword(@Nonnull PsiElement node)
	{
		...
		if(asyncNode != null)
		{
			registerOnFirstMatchingVersion(level -> level.isOlderThan(LanguageLevel.PYTHON35), "Python versions < 3.5 do not support this syntax", node, asyncNode.getTextRange(), null);
		}
	}
	...
	protected abstract void registerProblem(@Nonnull PsiElement node, @Nonnull TextRange range, @Nonnull String message, @Nullable LocalQuickFix localQuickFix, boolean asError);
	...
	protected void registerForAllMatchingVersions(@Nonnull Predicate<LanguageLevel> levelPredicate,
			...
			boolean asError)
	{
		final List<String> levels = myVersionsToProcess.stream().filter(levelPredicate).map(LanguageLevel::toString).collect(Collectors.toList());
	...
	protected void registerForAllMatchingVersions(@Nonnull Predicate<LanguageLevel> levelPredicate,
			...
			@Nullable LocalQuickFix localQuickFix)
	{
		final List<Pair<? extends PsiElement, TextRange>> nodesWithRanges = ContainerUtil.map(nodes, node -> Pair.createNonNull(node, node.getTextRange()));
	...
	protected void registerForAllMatchingVersions(@Nonnull Predicate<LanguageLevel> levelPredicate, @Nonnull String suffix, @Nonnull PsiElement node, @Nullable LocalQuickFix localQuickFix)
	{
	...
	protected void registerOnFirstMatchingVersion(@Nonnull Predicate<LanguageLevel> levelPredicate, @Nonnull String message, @Nonnull PsiElement node, @Nullable LocalQuickFix localQuickFix)
	{
		registerOnFirstMatchingVersion(levelPredicate, message, node, node.getTextRange(), localQuickFix);
	}

	protected void registerOnFirstMatchingVersion(@Nonnull Predicate<LanguageLevel> levelPredicate, @Nonnull String message, @Nonnull PsiElement node)
	{
	...
	private void highlightIncorrectArguments(@Nonnull PyCallExpression callExpression)
	{
		...
		for(PyExpression argument : callExpression.getArguments())
		{
			if(argument instanceof PyKeywordArgument)
			{
				...
				{
					registerOnFirstMatchingVersion(level -> level.isOlderThan(LanguageLevel.PYTHON26), "Python versions < 2.6 do not allow keyword arguments after *expression", argument, new
							PyRemoveArgumentQuickFix());
				}
				else if(seenKeywordContainer)
				{
					registerOnFirstMatchingVersion(level -> level.isOlderThan(LanguageLevel.PYTHON35), "Python versions < 3.5 do not allow keyword arguments after **expression", argument, new
							PyRemoveArgumentQuickFix());
				}
			...
			else if(argument instanceof PyStarArgument)
			{
				...
				if(starArgument.isKeyword())
				{
					...
					{
						registerOnFirstMatchingVersion(level -> level.isOlderThan(LanguageLevel.PYTHON35), "Python versions < 3.5 do not allow duplicate **expressions", argument, new
								PyRemoveArgumentQuickFix());
					}
			...
			else
			{
				...
				{
					registerOnFirstMatchingVersion(level -> level.isOlderThan(LanguageLevel.PYTHON35), "Python versions < 3.5 do not allow positional arguments after *expression", argument, new
							PyRemoveArgumentQuickFix());
				}
				else if(seenKeywordContainer)
				{
	...
	public void visitPyComprehensionElement(PyComprehensionElement node)
	{
		...
		{
			Arrays.stream(node.getNode().getChildren(TokenSet.create(PyTokenTypes.ASYNC_KEYWORD))).filter(Objects::nonNull).map(ASTNode::getPsi).forEach(element -> registerProblem(element, "Python " +
					"version 3.5 does not support 'async' inside comprehensions and generator expressions"));

			final Stream<PyPrefixExpression> resultPrefixExpressions = PsiTreeUtil.collectElementsOfType(node.getResultExpression(), PyPrefixExpression.class).stream();

			final Stream<PyPrefixExpression> ifComponentsPrefixExpressions = node.getIfComponents().stream().map(ifComponent -> PsiTreeUtil.collectElementsOfType(ifComponent.getTest(),
					PyPrefixExpression.class)).flatMap(Collection::stream);

			Stream.concat(resultPrefixExpressions, ifComponentsPrefixExpressions).filter(expression -> expression.getOperator() == PyTokenTypes.AWAIT_KEYWORD && expression.getOperand() != null).map
					(expression -> expression.getNode().findChildByType(PyTokenTypes.AWAIT_KEYWORD)).filter(Objects::nonNull).map(ASTNode::getPsi).forEach(element -> registerProblem(element, "Python" +
					" version 3.5 does not support 'await' inside comprehensions"));
		}
	}
}
